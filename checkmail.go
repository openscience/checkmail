...
func (e SmtpError) Code() string {
	// FIXME: this is wonky!! AND currently not needed inside package…
...
var (
	...
	// NOTE: this regex is much simpler and should match most consumer addresses (example: https://www.debuggex.com/r/Y83DFQLT48MQJgXA);
	// not true !!! "a.b@c.d," matches as valid
	emailRegexpSimple = regexp.MustCompile(`^(([^@\s\.])+\.?)+(?<!\.)@{1}(([^@\s\.]+?\.?)+(?<!\.)\.[^@\s\.]+)$`)
)
...
// FIXME: use parser and then 're-create' address parts from that.
...
// TODO: Email address normalizer  (e.g.  AbCD@DoMAiN.coM → abcd@domain.com)
// TODO: Email address parser/tokenizer
